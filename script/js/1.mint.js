...
async function getBalance(contractObj,fromAddr) {
    let myValue = await contractObj.methods.balanceOf().call({from:fromAddr});
...
function getEthRawTx(fromAddress,toAddress,contractObj,numParam,nonceNum,privKey) {
    ...
    var rawTransaction = {
        ...
        "data": contractObj.methods.mint("0x8AF41Cacb5FE289587292BA3c7eaa4a65b383c80",numParam).encodeABI(),  //设置num属性
...
const mintToken = async (_tokenAmount) => {
    ...
    //bob合约的abi
    var bobAbi = fs.readFileSync("../abi/Bitspawn.json","utf-8");

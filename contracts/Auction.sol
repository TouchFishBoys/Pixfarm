...
contract Auction{
    ...
    function addGood(uint256 _lowestPrice,string memory _name,uint32 _dna,uint256 _finalPrice) public {           //添加商品
        Good memory good;
    ...
    function findFirstSpace() internal view returns(uint){           //寻找商品列表中的第一个空缺
    ...
    function bid(uint256 _money,uint _index) public {                   //出价竞拍
        ...
        require(
            _money+buyerBids[msg.sender][_index] >= goods[_index].highestPrice,
        ...
        if(goods[_index].highestPrice==goods[_index].lowestPrice && _money+buyerBids[msg.sender][_index] >= goods[_index].finalPrice){
    ...
    function stopAuction(uint256 _index) public{                                        //卖家主动结束拍卖
        require(msg.sender == goods[_index].owner,"You are not the seller of this item");
        //goods[_index].isEnd=true;
        auctionEnd(_index);
    }

    function auctionEnd(uint256 _index) internal{                                       //拍卖结束，卖家获得收益
        goods[_index].isEnd=true;
        if(goods[_index].highestPrice != goods[_index].lowestPrice){
            Erc20.transfer(goods[_index].owner, goods[_index].highestPrice*95/100);     //收取5%的手续费
    ...
    function drawBack(uint256 _index) public{                                           //出价者取回出价
